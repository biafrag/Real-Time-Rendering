...
void main()
{
    vec3 ambient = material.ambient *vec3(1,1,1);//texture(sampler, fragUV).rgb; //Componente da luz ambiente
    ...
    if( iDif > 0 )
    {
        diffuse = iDif * material.diffuse *vec3(1,1,1);//texture(sampler, fragUV).rgb;// Calcula componente difusa da luz
